...
#### Vetor Simétrico
- Possui mesmo módulo, porém sentido oposto;
...
- Um **<ins>Vetor<ins>** é um segmento de reta orientado que possui [comprimento](#módulo-norma-ou-comprimento), [direção](#direção) e [sentido](#sentido).
    - ***Vetor unitário*** - possui módulo $|v|$ igual a 1. [Mais informações aqui](#vetor-unitário)

### Operações com Vetores
#### Soma de vetores
- Usa a [Regra do Paralelogramo](https://pt.wikipedia.org/wiki/Regra_do_paralelogramo):  
...
- É dado por $a$ - $b$. Para obter $-b$ basta inverter o seu [sentido](#sentido).
...
- Dado um valor real $k$ a multiplicação é de forma direta $u=kv$.  
- Possui as propriedades:  
    - **Módulo**: $|u|=|kv|=|k| \cdot |v|$  
        - exemplo: $|u|= |2| \cdot \sqrt{3^2+4^2} = 2 \cdot \sqrt{25} = 10$  
    - **Direção:** O vetor resultante é paralelo ao vetor original (mantém a mesma direção);  
    - **Sentido:** Se $k>0$, o sentido é o mesmo; se $k<0$, o sentido é invertido.
...
- Caso posicionemos um vetor $v$ em cima de um vetor $u$, podemos criar uma sombra de $v$ em $u$, ou [projeção ortogonal](#projeção-ortogonal).
    ...
    - Nesse caso em particular o resultado da projeção será o própio vetor $v$, pois ele está em cima da projeção $u$;  
